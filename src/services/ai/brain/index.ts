...
import { smartUnpack, formatSize, DiffData, VibeVault } from '../../../core/vault';
...
export async function generateCommitMessage(input: BrainInput): Promise<BrainOutput> {
  ...
  if (diffData && diffData.mode === 'ref' && diffData.dataRef) {
    return VibeVault.withAutoCleanup(diffData.dataRef, () =>
      runBrain({ ...input, diff, diffData })
    );
  }
...
async function runBrain(input: BrainInput): Promise<BrainOutput> {
  const { diff, diffData, hint, language, debug, think: thinkFlag } = input;
